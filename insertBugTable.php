...
<?php
    ...
    if(isset($_POST['submit']))
    {
        ...
        if( empty($_POST['prog_id']) || empty($_POST['reported_by']) || empty($_POST['report_type']) || empty($_POST['severity'])|| empty($_POST['reproducible'])|| empty($_POST['problem_summary'])|| empty($_POST['problem'])|| empty($_POST['date']))
        {
        ...
        else
        {
            ...
            $query = " INSERT INTO bugtable (prog_id,area_id,report_type,severity,problem_summary,problem,reproducible,reported_by,date,suggested_fix,assigned_to,comments,status,priority,resolution,resolution_version,resolved_by,resolved_date,tested_by,test_date,attachment,attachment_type,attachment_size) 
                                    values($prog_id,$area_id,'$report_type','$severity','$problem_summary','$problem','$reproducible',$reported_by,'$date','$suggested_fix',$assigned_to,'$comments','$status','$priority','$resolution','$resolution_version',$resolved_by,'$resolved_date',$tested_by,'$test_date','$attachment','$attachment_type','$attachment_size')";
